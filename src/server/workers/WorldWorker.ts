...
const workerData = {
	generateWorld: (width: number, height: number, seed?: string) => {
		...
		for (let x = 0; x < width; x++) {
			...
			if (y >= height) continue; //if the tile that's being changed is below the world, give up before any issues happen
			if (tiles[y * width + x] === TileType.Ice) {
				//ice
		...
		for (let t = 0; t < width; t++) {
			...
			for (let i = 0; i < height - 1; i++) {
				if (
					(tiles[(i + 1) * width + x + 1] === TileType.Dirt ||
						tiles[(i + 1) * width + x + 1] === TileType.Snow) &&
					(tiles[(i + 1) * width + x + 2] === TileType.Dirt ||
						tiles[(i + 1) * width + x + 2] === TileType.Snow)
				) {
					let isOpen = true;
					for (let j = 0; j < 4; j++) {
						for (let k = 0; k < 6; k++) {
							...
							if (
								!inWorld(x + j, i - k, width, height) ||
								tiles[(i - k) * width + x + j] !== TileType.Air
							) {
								isOpen = false;
								//break;
							}
