            ...
            for (
                ...
                if (game.world.worldTiles[j * game.worldWidth + i] !== 0) {
                    ...
                    if (
                        ...
                        (this.closestCollision[2] > this.collisionData[2] || // the "closest collision" is the one that occurs after the least amount of time during the frame

                        /* in case of a tie (this can occur while running into a wall with multiple blocks)
                        having it also be the closest physically at the same time fixes an issue with corners, so let's do that.*/

                            this.collisionData[2] === this.closestCollision[2] &&
                            abs(this.x - this.collisionData[0]) + abs(this.y - this.collisionData[1]) < abs(this.x - this.closestCollision[0]) + abs(this.y - this.closestCollision[1]))// this "distance" function works because collisions are axis-aligned and any collision in a direction causes the entity to move in an axis-aligned direction, which works with this distance function
