...
class EntityBase {
    ...
    applyGravityAndDrag() {
        ...
        this.xVel -=
            (Main_1.game.abs(this.xVel) * this.xVel * Main_1.game.DRAG_COEFFICIENT * this.w) /
                this.mass;
        this.yVel += Main_1.game.GRAVITY_SPEED;
        this.yVel -=
            (Main_1.game.abs(this.yVel) * this.yVel * Main_1.game.DRAG_COEFFICIENT * this.h) /
                this.mass;
    }
    findInterpolatedCoordinates() {
        ...
        if (this.pXVel > 0) {
            this.interpolatedX = Main_1.game.min(this.pX + this.pXVel * Main_1.game.amountSinceLastTick, this.x);
        }
        else {
            this.interpolatedX = Main_1.game.max(this.pX + this.pXVel * Main_1.game.amountSinceLastTick, this.x);
        }
        if (this.pYVel > 0) {
            this.interpolatedY = Main_1.game.min(this.pY + this.pYVel * Main_1.game.amountSinceLastTick, this.y);
        }
        else {
            this.interpolatedY = Main_1.game.max(this.pY + this.pYVel * Main_1.game.amountSinceLastTick, this.y);
        }
    }
    applyVelocityAndCollide() {
        ...
        */
        ...
        for (let i = Main_1.game.floor(Main_1.game.min(this.x, this.x + this.xVel)); i < Main_1.game.ceil(Main_1.game.max(this.x, this.x + this.xVel) + this.w); i++) {
            for (let j = Main_1.game.floor(Main_1.game.min(this.y, this.y + this.yVel)); j < Main_1.game.ceil(Main_1.game.max(this.y, this.y + this.yVel) + this.h); j++) {
                // if the current tile isn't air (value 0), then continue with the collision detection
                if (Main_1.game.world.worldTiles[j * Main_1.game.WORLD_WIDTH + i] !== 0) {
                    // get the data about the collision and store it in a variable
                    this.collisionData = Main_1.game.rectVsRay(i - this.w, j - this.h, this.w + 1, this.h + 1, this.x, this.y, this.xVel, this.yVel);
        ...
        if (this.closestCollision[2] !== Infinity) {
            ...
            this.closestCollision = [1, 0, Infinity];
            // loop through all the possible tiles the physics box could be intersecting with
            for (let i = Main_1.game.floor(Main_1.game.min(this.x, this.x + this.slideX)); i < Main_1.game.ceil(Main_1.game.max(this.x, this.x + this.slideX) + this.w); i++) {
                for (let j = Main_1.game.floor(Main_1.game.min(this.y, this.y + this.slideY)); j <
                    Main_1.game.ceil(Main_1.game.max(this.y, this.y + this.slideY) + this.h); j++) {
                    // if the current tile isn't air (value 0), then continue with the collision detection
                    if (Main_1.game.world.worldTiles[j * Main_1.game.WORLD_WIDTH + i] !== 0) {
                        // get the data about the collision and store it in a variable
                        this.collisionData = Main_1.game.rectVsRay(i - this.w, j - this.h, this.w + 1, this.h + 1, this.x, this.y, this.slideX, this.slideY);
                        if (this.collisionData !== false &&
                            this.closestCollision[2] > this.collisionData[2]) {
                            this.closestCollision = this.collisionData;
                            Main_1.game.tempcol = [
        ...
        else if (this.x + this.w > Main_1.game.WORLD_WIDTH) {
            this.x = Main_1.game.WORLD_WIDTH - this.w;
            this.xVel = 0;
        }
        if (this.y < 0) {
            this.y = 0;
        ...
        else if (this.y + this.h > Main_1.game.WORLD_HEIGHT) {
            this.y = Main_1.game.WORLD_HEIGHT - this.h;
            this.yVel = 0;
            this.grounded = true;
        }
    }
