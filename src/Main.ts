...
class Game extends P5 {
    ...
    BACK_TILE_WIDTH = 12 // width of a tile in pixels
    BACK_TILE_HEIGHT = 12 // height of a tile in pixels
    TILE_SET_SIZE = 16 // number of tiles in the tile set (do not change this as parts of the program will break)

    backTileLayerOffsetWidth = -2 // calculated in the setup function, this variable is the number of pixels left of a tile to draw a background tile from at the same position
    backTileLayerOffsetHeight = -2 // calculated in the setup function, this variable is the number of pixels above a tile to draw a background tile from at the same position

    upscaleSize = 6 // number of screen pixels per texture pixels (think of this as hud scale in Minecraft)

    worldTiles = new Uint8Array(this.WORLD_WIDTH * this.WORLD_HEIGHT) // number for each tile in the world ex: [1, 1, 0, 1, 2, 0, 0, 1...  ] means snow, snow, air, snow, ice, air, air, snow...
    backgroundTiles = new Uint8Array(this.WORLD_WIDTH * this.WORLD_HEIGHT) // number for each background tile in the world ex: [1, 1, 0, 1, 2, 0, 0, 1...  ] means snow, snow, air, snow, ice, air, air, snow...

    camX = 0 // position of the top left corner of the screen in un-up-scaled pixels (0 is left of world) (WORLD_WIDTH*TILE_WIDTH is bottom of world)
    camY = 0 // position of the top left corner of the screen in un-up-scaled pixels (0 is top of world) (WORLD_HEIGHT*TILE_HEIGHT is bottom of world)
    pCamX = 0 // last frame's x of the camera
    pCamY = 0 // last frame's y of the camera
    interpolatedCamX = 0 // interpolated position of the camera
    interpolatedCamY = 0 // interpolated position of the camera

    // tick management
    msSinceTick = 0 // this is the running counter of how many milliseconds it has been since the last tick.  The game can then
    msPerTick = 20 // the number of milliseconds per game tick.  1000/msPerTick = ticks per second
    amountSinceLastTick = 0 // this is a variable calculated every frame used for interpolation - domain[0,1)
    forgivenessCount = 50 // if the computer needs to do more than 50 ticks in a single frame, then it could be running behind, probably because of a freeze or the user being on a different tab.  In these cases, it's probably best to just ignore that any time has passed to avoid further freezing

    // physics constants
    GRAVITY_SPEED = 0.04 // in units per second tick, positive means downward gravity, negative means upward gravity
    DRAG_COEFFICIENT = 0.21 // arbitrary number, 0 means no drag
    ...
    drawTile(j?: any, i?: any) {
        if (this.worldTiles[i * this.WORLD_WIDTH + j] !== 0) {
            // test if the neighboring tiles are solid
            const topTileBool =
                i === 0 || this.worldTiles[(i - 1) * this.WORLD_WIDTH + j] !== 0
            const leftTileBool =
            ...
            this.tileLayer.image(
                ...
                this.TILE_WIDTH,
                this.TILE_HEIGHT,
                this.TILE_SET_POSITIONS[this.worldTiles[i * this.WORLD_WIDTH + j]] *
                    this.TILE_WIDTH *
                    this.TILE_SET_SIZE +
    ...
    doTick() {
        ...
        if (this.camX < 0) {
            this.camX = 0
        } else if (this.camX > this.WORLD_WIDTH - this.width / this.upscaleSize / this.TILE_WIDTH) {
            this.camX = this.WORLD_WIDTH - this.width / this.upscaleSize / this.TILE_WIDTH
        }
        if (this.camY > this.WORLD_HEIGHT - this.height / this.upscaleSize / this.TILE_HEIGHT) {
            this.camY = this.WORLD_HEIGHT - this.height / this.upscaleSize / this.TILE_HEIGHT
        }
        ...
        for (const item of this.physicsRects) {
            if (item.type === 'item') {
        ...
        for (let i = 0; i < this.physicsRects.length; i++) {
            if (this.physicsRects[i].deleted === true) {
                this.physicsRects.splice(i, 1)
                i--
            }
        }
    }
  ...
  */
    ...
    dropItem(type?: any, count?: any, x?: any, y?: any) {
        for (let i = 0; i < count; i++) {
            ...
            this.physicsRects[this.physicsRects.length - 1].type = 'item'
            this.physicsRects[this.physicsRects.length - 1].itemType = type
        }
    }
    ...
    updateMouse() {
        ...
        this.worldMouseX = this.floor(
            (this.interpolatedCamX * this.TILE_WIDTH + this.mouseX / this.upscaleSize) /
                this.TILE_WIDTH
        )
        this.worldMouseY = this.floor(
            (this.interpolatedCamY * this.TILE_HEIGHT + this.mouseY / this.upscaleSize) /
                this.TILE_HEIGHT
        )
    }
    ...
    drawCursor() {
        if (this.mouseOn) {
            ...
            this.cursorAnimFrame =
                this.cursorAnimFrame % this.assets.cursors[this.cursorIndex].length
            this.image(
